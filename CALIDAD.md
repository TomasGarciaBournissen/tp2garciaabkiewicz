...
Usamos Claude Code (agente de IA de Anthropic) para implementar el pipeline de CI/CD, los tests y esta documentación. No fue solo "generar tests sueltos": el agente armó la extracción de lógica de negocio a `src/lib/`, la configuración de Vitest y Playwright, el workflow de GitHub Actions y la conexión con Vercel, siguiendo decisiones que tomamos nosotros en el camino (por ejemplo: mockear Supabase en el E2E en vez de apuntar a un proyecto real, o desconectar el auto-deploy de Vercel para que el pipeline de CI sea el único gate a producción). Revisamos el resultado antes de la entrega — código, workflow y este mismo documento — y podemos explicar en la defensa qué hace cada test, cómo está armado el pipeline y qué trade-offs aceptamos conscientemente (ver la sección siguiente).
...
- **No se implementó ninguno de los extras opcionales** (Sentry, coverage con umbral, GitHub Projects) salvo el PR template. Quedan como mejora futura si el tiempo lo permite.
